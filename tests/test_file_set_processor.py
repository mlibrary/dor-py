...
from dor.providers.process_basic_image import FileSetIdentifier, process_basic_image, ServiceImageTransformer
...
@pytest.fixture
def service_image_transformer(output_path: Path, file_set_identifier: FileSetIdentifier) -> ServiceImageTransformer:
    return ServiceImageTransformer(
...
def test_process_basic_image_copy_copies_input_file_to_output_file(
    file_set_identifier, input_path, output_path, service_image_transformer
):
    ...
    assert process_basic_image(
        ...
        transformers=[service_image_transformer]
    )
    assert copy_of_source_file.exists()


def test_process_basic_image_creates_technical_metadata(
    file_set_identifier, input_path, output_path, service_image_transformer
):
...
def test_process_basic_image_creates_service_image(
    file_set_identifier, input_path, output_path, service_image_transformer
):
...
def test_process_basic_image_creates_service_technical_metadata(
    file_set_identifier, input_path, output_path, service_image_transformer
):
    technical_metadata_file = output_path / file_set_identifier.identifier / "metadata" / \
...
def test_process_basic_image_creates_descriptor_file(
    file_set_identifier, input_path, output_path, service_image_transformer
):
...
def test_process_basic_image_creates_service_event_metadata(
    file_set_identifier, input_path, output_path, service_image_transformer
):
...
def test_process_basic_image_creates_source_event_metadata(
    file_set_identifier, input_path, output_path, service_image_transformer
):
    event_metadata_file = output_path / file_set_identifier.identifier / "metadata" / \
